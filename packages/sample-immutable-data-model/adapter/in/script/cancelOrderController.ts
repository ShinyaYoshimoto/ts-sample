...
import type { CancelOrderUseCase } from '../../../application/port/in/cancelOrderUseCase';
...
rl.on('close', () => {
	...
	const orderCancellationPersistenceAdapter =
		new OrderCancellationPersistenceAdapter(prisma);
	const cancelOrderService = new CancelOrderService(
		orderCancellationPersistenceAdapter,
	);
	...
	new CancelOrderController(cancelOrderService, getOrderService).handle(
		request,
	);
});
