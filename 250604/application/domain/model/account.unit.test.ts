...
describe("Account", () => {
  ...
  describe("constructor", () => {
    it("should create an account with given parameters", () => {
      ...
      const account = new Account(accountId, baselineBalance, mockActivityWindow);
  ...
  describe("calculateBalance", () => {
    it("should return sum of baseline balance and activity window balance", () => {
      const accountId = "account-1";
      const baselineBalance = new Money(500);
      (mockActivityWindow.calculateBalance as any).mockReturnValue(new Money(200));
      
      const account = new Account(accountId, baselineBalance, mockActivityWindow);
      const balance = account.calculateBalance();
      
      expect(balance.amount).toBe(700);
      expect(mockActivityWindow.calculateBalance).toHaveBeenCalledWith(accountId);
    });
  ...
  describe("withdraw", () => {
    it("should successfully withdraw when sufficient balance", () => {
      ...
      (mockActivityWindow.calculateBalance as any).mockReturnValue(new Money(0));
    ...
    it("should create activity with correct timestamp", () => {
      ...
      const addedActivity = (mockActivityWindow.addActivity as any).mock.calls[0][0];
      expect(addedActivity.timestamp.getTime()).toBeGreaterThanOrEqual(beforeWithdraw.getTime());
      expect(addedActivity.timestamp.getTime()).toBeLessThanOrEqual(afterWithdraw.getTime());
    });
  });
  ...
  describe("deposit", () => {
    ...
    it("should create deposit activity with correct timestamp", () => {
      ...
      const addedActivity = (mockActivityWindow.addActivity as any).mock.calls[0][0];
      expect(addedActivity.timestamp.getTime()).toBeGreaterThanOrEqual(beforeDeposit.getTime());
      expect(addedActivity.timestamp.getTime()).toBeLessThanOrEqual(afterDeposit.getTime());
    });
  });
